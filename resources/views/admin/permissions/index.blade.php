                        ...
                        <table id="basicExample" class="table table-striped table-bordered">
                            ...
                            @if(count($permissions))
                                @foreach($permissions as $permission)
                                    <tr data-id="{{$permission->id}}" id="id{{$permission->id}}">
                                        <td>{{$permission->name}}</td>
                                        <td>{{ date('d-m-Y h:i A',strtotime($permission->created_at)) }}</td>
                                        <td>{{ timeago($permission->updated_at, 'en')}}</td>
                                        <td>
                                            <a href="javascript:void(0);" class="btn btn-primary btn-sm edit"  data-id="{{$permission->id}}">
                                                <span class="icon-pencil"></span>
                                            </a>
                                            <a href="javascript:void(0);" class="btn btn-danger btn-sm deletej" onclick="deletePermission({{$permission->id}});" data-id="{{$permission->id}}">
...
@endsection
@section('scripts')
    <script type="text/javascript" src="{{asset('admin/custom/js/permission.js')}}"></script>
