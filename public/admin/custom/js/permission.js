...
$(document).on('submit', '#add_form', function(e){
	 e.preventDefault();
   var url    = base_url + '/admin/permission',
       method = 'POST',
       data   = {'name': $('#name').val()},
      modal = '#addModal';
        ...
        if(data.success == true){          
          ...
          var tr ='<tr data-id="'+data.result.id+'" id="id'+data.result.id+'">'+
            ...
            '<td><a href="javascript:void(0);" class="btn btn-primary btn-sm edit"   data-id="'+data.result.id+'" style="margin-right: 4px;">'+
                  ...
                  '</a>'+
                  '<a href="javascript:void(0);" class="btn btn-danger btn-sm"  onclick="deletePermission('+data.result.id+');"data-id="'+data.result.id+'">'+
...
$(document).on('submit', '#edit_form',function(e){
   e.preventDefault();
 var id = $('#edit_id').val(),
     url = base_url + '/admin/permission/'+id,
     method = 'PUT',
     data   = {
        'id'     : $('#edit_id').val(),
        'name'   : $('#edit_name').val(),
...
function getData(id){
     $.ajax({
      ...
      success : function(data){
          if(data.success == true){
             $('#edit_id').val(data.permission.id);
...
function deletePermission(id){
    	...
    	Swal({
    		...
    		if(result.value){
    			var hitURL = base_url +'/admin/permission/'+id;
					jQuery.ajax({
					type     : "DELETE",
					dataType : "json",
					url      : hitURL,
